...
import { ShoppingCart, Heart, X, MapPin, Package, Filter, SlidersHorizontal, XCircle, Search, FilePlus, ClipboardList, FileText, Users, File, Trash2, RotateCcw, Pencil, Loader2 } from "lucide-react"
...
export function DashboardClient({ productos, userName, userRole }: Props) {
        ...
        for (let i = 0; i < localStorage.length; i++) {
            ...
            if (key?.startsWith("pedido-borrador-")) {
                try {
                    ...
                    const userName = key.replace("pedido-borrador-", "")
                    lista.push({ userName, data, fecha: data.fecha || "" })
                } catch (e) {
    ...
    const restaurarBorrador = (userName: string) => {
        const key = `pedido-borrador-${userName}`
        localStorage.setItem("pedido-borrador-restore", key)
        setShowBorradores(false)
        setShowCrearPedido(true)
    }

    const eliminarBorrador = (userName: string) => {
            ...
            <div className="p-4 md:p-6 lg:p-10 font-sans pb-20 md:pb-10">
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        ...
                        <button
                            onClick={() => { cargarPedidosAsignados(); setShowModificarPedido(true) }}
                        ...
                        <button
                            onClick={() => { cargarBorradores(); setShowBorradores(true) }}
                ...
                <MobileNav
                    userName={userName}
                    userRole={userRole}
                    onOpenCrearPedido={() => { setPedidoAEditar(null); setShowCrearPedido(true) }}
                    onOpenBorradores={() => { cargarBorradores(); setShowBorradores(true) }}
                    onOpenModificarPedido={() => { cargarPedidosAsignados(); setShowModificarPedido(true) }}
                />
            </div>
        )
    }
